...
// LADRC 控制器实例数组
static LADRC_t motor_ladrc[M2006_MOTOR_NUM];

// 目标转速缓存
static int16_t target_rpm[M2006_MOTOR_NUM] = {0};

// 控制输出缓存
static int16_t control_output[M2006_MOTOR_NUM] = {0};
...
static void Create_LADRC_Config(ladrc_config_t *config) {
  ...
  // LESO 参数
  config->w0 = M2006_LESO_W0; // 观测器带宽
  config->b0 = M2006_LESO_B0; // 控制增益

  // PD 参数（一阶只用 kp）
  config->kp = M2006_CONTROL_KP; // 比例系数
  config->kd = 0.0f;             // 一阶不使用微分

  // 输出限幅
  config->maxout = M2006_MAX_OUTPUT;
  ...
  // 优化参数
  config->feedback_LPF_RC = M2006_FEEDBACK_LPF_RC;
  config->disturbance_limit = M2006_DISTURBANCE_LIMIT;
  config->disturbance_decay = M2006_DISTURBANCE_DECAY;
}
...
void M2006_RPM_Init(void) {
  ...
  // 初始化每个电机的 LADRC 控制器
  for (uint8_t i = 0; i < M2006_MOTOR_NUM; i++) {
    LADRC_Init(&motor_ladrc[i], &config, M2006_CONTROL_DT);
...
void M2006_Set_RPM(int16_t rpm1) { target_rpm[0] = rpm1; }
...
void M2006_RPM_Control_Loop(void) {
  ...
  for (uint8_t i = 0; i < M2006_MOTOR_NUM; i++) {
    // 获取电机反馈数据
    measure = get_trigger_m2006_measure_point();
    if (measure == NULL) {
      continue;
    }
  ...
  // 发送 CAN 命令
  motor_control_t motors[1];
  motors[0].slot = MOTOR_SLOT_0;
  for (int i = 0; i < 4; i++) {
    motors[i].msg_id = CAN_MSG_M2006_ID;
    motors[i].value = (int16_t)control_output[i];
  }

  CAN_Manager_SetMotors(motors, 1);
}

int16_t M2006_Get_Actual_RPM(uint8_t motor_index) {
  if (motor_index >= M2006_MOTOR_NUM) {
    return 0;
  }

  const motor_measure_t *measure = get_trigger_m2006_measure_point();
